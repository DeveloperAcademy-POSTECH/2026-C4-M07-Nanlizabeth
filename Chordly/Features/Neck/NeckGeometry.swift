...
enum NeckGeometry {
    ...
    ///
    /// - Note: `iPhoneNeckBackground` 이미지(2622×1206 = 스테이지 3배, **인레이 없는 넥**)에서
    ///   프렛 바 중심을 측정해 맞춘 값이다. 배경 이미지를 바꾸면 이 값도 다시 맞춰야 짚는 점·목표
    ///   표식·인레이가 그림 속 프렛과 어긋나지 않는다.
    static let fretBoundaryXs: [CGFloat] = [827, 650, 474, 306, 148, 16]
