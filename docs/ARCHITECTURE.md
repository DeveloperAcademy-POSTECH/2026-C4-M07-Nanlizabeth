...
### 3.4 `FretboardInput` ↔ `FingeringState` [새로] — Domain/Fingering ★운지↔UI 이벤트 분리

> 🔁 **정책 변경 (2026-07-21):** 모드 A의 넥은 이제 **`.silent`** 로 쓴다 — 짚어도 소리가 안 나고,
> 소리는 자동 스트로크가 낸다 (SPEC §4). 아래 계약은 그대로다(`.pluckOnPress`도 여전히 지원) —
> **정책만 바뀌었을 뿐 구조는 안 바뀐다.** 이게 "정책을 값으로 뺀" 설계의 이점이다.

**책임:** 기타넥 터치를 받아 ①현재 운지 상태를 관리하고 ②정책에 따라 (원하면) 그 줄을 즉시 발음시키고 ③UI가 그릴 이벤트를 되돌려준다.
