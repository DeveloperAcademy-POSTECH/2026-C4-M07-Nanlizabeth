...
> ✅ **Phase 2에서 끝난 것:** U7 연결 화면(초안) · A4 오디오 중단 대응(PR 리뷰 중) · **U2 기타넥 화면**(`Features/Neck/` — `NeckGeometry`·`MultiTouchFretboardView`·`NeckViewModel`·`NeckScreen`).
>
...
| 방향 | **세로 고정** (`Info.plist`) + `PortraitLockedLandscapeStage`가 -90° 회전 → **기존 방식 공식화** |
| 기준 해상도 | **874×402** (Figma HI-FI iPhone 프레임 = iPhone 16 Pro/17 가로) = `LayoutTokens.referenceStage` |
| 반응형 정책 | **기준 해상도 + 비율 스케일(fit).** 기기별 재배치 없음 — 스테이지가 통째로 축소/확대 |
| 화면 담당이 할 일 | 없음. **874×402에 좌표를 잡으면 끝.** 기기 분기 금지 |

> ⚠️ **iPad는 이 규약으로 커버되지 않습니다 (2026-07-20 발견, 결정 대기).**
> Figma HI-FI에 **iPad Pro 12.9" 전용 프레임(1366×1024)** 이 따로 있고, 같은 화면을 4:3에 맞춰
> **다시 배치한 것**입니다(사운드홀이 중앙에 크게, 줄이 화면 전체를 가로지름). 단순 확대가 아닙니다.
> 가로세로비가 iPhone 2.17 : iPad 1.33이라 비율 스케일만 쓰면 iPad에서 위아래가 크게 비어 보입니다.
> **iPad는 모드 C의 소리 나는 쪽**이라 무시할 수 없는 기기입니다 → SPEC §8 결정 안건.
