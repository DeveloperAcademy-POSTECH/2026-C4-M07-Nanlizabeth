...
> ## ✅ 진행 상황 (2026-07-20)
>
> **Phase 0·1 완료 → Phase 2 진행 중.** 아래 4건이 `develop`에 병합됐습니다 (PR #4~#7).
>
> | 태스크 | 상태 |
> |--------|------|
> | **A4** 오디오 세션 안정성 | ✅ 병합 · ⏳ 실기기 검증 미완 |
> | **U2** 기타넥 화면 (멀티터치 개별 발음) | ✅ 병합 · ⏳ 실기기 멀티터치 검증 미완 |
> | **U3 일부** 스트럼 멀티터치 입력 | ✅ 병합 · ⏳ 실기기 검증 미완 (줄 애니메이션·iPad 레이아웃은 남음) |
> | **U4** 스트로크 선택 화면 | ✅ 병합 · 시뮬레이터 검증 완료 |
> | **U7** 연결 가이드·기기 찾기 | ✅ 초안 (디자인 확정 대기) |
>
> ⚠️ **병합됐다고 검증된 건 아닙니다.** 위 ⏳ 항목은 전부 **하드웨어에서만 재현되는 기능**이라 시뮬레이터로 확인이 불가능합니다. 각 태스크의 체크리스트가 아래 표 밑에 있습니다.
>
> **실기기 테스트에서 잡힌 버그 2건** (둘 다 수정·병합됨):
> - **화면 왕복 후 기타가 사인파로 바뀜** — `engine.stop()`이 샘플러의 사운드폰트를 날렸다. `pause()`로 교체. **A4의 중단 복구 경로에도 같은 결함이 있었다** (전화 후 복귀해도 사인파가 났을 상황).
> - **여러 줄을 동시에 못 튕김** — 입력이 `DragGesture`라 손가락을 하나만 받았다. 넥·스트럼 모두 공용 `MultiTouchLayer`로 교체.
>
...
>
> 📌 **회의 안건: L1은 이미 끝난 것 아닌가?** 로직 쪽(멀티터치 → 운지 갱신 + `pluckOnPress` + `notePlayed` 방송)은 **C3가 실구현으로 냈고**, UI 쪽(터치 → `FretPress` 변환)은 **U2가 했습니다.** 별도 태스크로 남겨둘 이유가 있는지 확인이 필요합니다.
>
> ⚡ **L2·I1 진행 (이후 추가):** 스트럼 플레이어(L2, `StrumPatternPlayer`)와 모드 A 조립(I1, `ChordPracticeSession`)이 들어왔습니다. **모드 A가 로직 수준에서 실제로 성립합니다** — 짚은 코드가 자동 주법대로 긁히고, 연주 중 코드를 바꾸면 다음 스트럼부터 반영됩니다(기록형 가짜 엔진으로 검증). 남은 건 **재생 버튼을 이 세션에 연결**하는 화면 작업뿐인데, 프로토타입 셸(`MainInstrumentScreen`)이 곧 교체될 예정이라 버릴 코드를 안 만들려고 연결은 U 레인 정리와 함께 하기로 미뤄뒀습니다.
