...
> | 🔧 조립 | **I1** 모드 A · **I2** 모드 B · **I3** 모드 C (전부 세션 객체로 조립) | ✅ 자체검증 |
>
...
> ### 아직 안 된 것
>
> - **화면↔세션 연결**: 세 모드 세션(`ChordPracticeSession`·`StrumPracticeSession`·`EnsembleStrummerSession`)이 **아직 재생 버튼·긁기 입력에 안 물렸습니다.** 프로토타입 셸(`MainInstrumentScreen`)이 교체될 때 연결합니다. **이걸 해야 앱에서 실제로 소리가 납니다.**
...
> - **H2/H3** (Core Haptics 감쇠·판정) · **H4** (접근성 전반)
