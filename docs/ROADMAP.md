...
> ## ✅ 진행 상황 (2026-07-20 기준, PR #4~#13 병합)
>
> **Phase 2 대부분 완료 + 세 모드 로직 조립 완료.** 시뮬레이터 빌드 통과.
>
> ### 완료 (develop 병합됨)
>
> | 영역 | 태스크 | 검증 |
> |------|--------|------|
> | 🎵 소리 | **A4** 오디오 세션 복구 + 사인파 버그 수정 | ⏳ 실기기 |
> | 🧠 로직 | **L2** 스트럼 플레이어 · **L3** 진행 플레이어 · **L4** 미리듣기 | ✅ 자체검증 |
> | 🧠 로직 | **L5** 소스·코디네이터 (C6에서 이미 완성돼 있었음) | ✅ |
> | 🖼️ 화면 | **U1** 온보딩 · **U2** 기타넥 · **U4** 스트로크선택 · **U5** 진행선택 · **U6** 진행커스텀 · **U7** 연결 | ✅ 시뮬 / ⏳ 실기기 |
> | 🖼️ 화면 | **U3(입력)** 스트럼 멀티터치 | ⏳ 실기기 |
> | 📡 통신 | **N1** 원격 소스 배선 | ✅ 자체검증 |
> | 🫨 손맛 | **H1** 발음 햅틱 (넥 연결) | ⏳ 실기기 |
> | 🔧 조립 | **I1 모드 A** (✅ 화면 연결까지 — 재생 버튼이 실제로 자동 스트럼) · **I2** 모드 B · **I3** 모드 C (세션 조립) | ✅ 자체검증 |
>
> **세 모드가 로직 수준에서 전부 성립합니다** — 기록형 가짜 엔진 + `MockBeatClock`으로 체인을 검증했습니다 (시뮬레이터로는 소리를 못 들으니 이게 더 확실). 자체 검증이 실제 버그 3개를 잡았습니다 (사인파, 단일터치, 한번재생 종료 타이밍).
>
> ### 아직 안 된 것
>
> - **화면↔세션 연결**: **모드 A(iPhone)는 연결 완료** — 넥에서 짚고 재생하면 자동 스트럼이 돈다(`ChordModeController`). 남은 건 **모드 B/C(스트럼 화면 쪽)** 연결인데, 스트럼 화면 재작성 + iPad 레이아웃(SPEC §8)과 함께 한다.
> - **U3 스트럼 화면 시각화 + iPad 레이아웃**: 줄 애니메이션은 넥에 넣었고, 스트럼 화면 자체(이미지→벡터)와 iPad 재배치는 **SPEC §8 iPad 결정** 후.
> - **실기기 검증 누적**: A4·U2·U3·H1 + 세 모드 소리. 하드웨어에서만 재현됩니다.
> - **H4** (접근성 전반) — 다이내믹 타입은 가능, VoiceOver 순서는 기기
> - **H3 판정 햅틱**: 메커니즘 완료. **어느 모드에서 켤지 SPEC §8 결정**만 남음 (모드 A엔 정답이 없어 기본 꺼짐)
> - **A1** `gs_instruments.dls`(에셋 필요) · **A2** 지연 계측(기기 필요) · **A3** 상시 수집
> - **CT1/CT2/CT3** 콘텐츠 리서치 (리서치 담당 레인 — 코드는 다 받을 준비됨)
> - **I4** 통합 QA · **I5** 엔진 최종 결정 (출시 직전)
>
> ### Phase 0 잔여 (다른 작업 안 막음)
> - **F2 여백 임시값**: `SpacingTokens.swift`만 임시. Figma 여백 스펙 확보 후 숫자만 교체.
> - **F5 팀 승인**: 온보딩 정책 회의 승인만 남음.
>
> 📌 **회의 안건 — L1은 사실상 완료:** 로직(C3)·UI(U2)가 이미 덮었습니다. 별도 태스크 유지 여부 확인 필요.
...
| ID | 태스크 | 하는 일 | 관련 계약 | 선행 | 크기 |
|----|--------|--------|----------|------|------|
| L1 | 운지 상태 구현 | 멀티터치 → 운지 갱신 + 개별 발음(`pluckOnPress`) + `notePlayed` 방송 | §3.4 | C3 | ❓ C3+U2가 이미 덮음 — 회의 확인 |
| L2 | 스트럼 플레이어 | 패턴을 클럭에 맞춰 자동 연주 (모드 A의 자동 오른손) | §3.5 | C4 | ✅ 완료 (`StrumPatternPlayer`) |
| L3 | 진행 플레이어 | 진행을 클럭에 맞춰 자동 운지 (모드 B의 자동 왼손) | §3.6 | C5 | ✅ 완료 (`ChordProgressionPlayer`) |
| L4 | 미리듣기 플레이어 | 기본 주법·BPM으로 재생, 동시 재생 방지 | §3.6 | C5, L2 | ✅ 완료 (`ProgressionPreviewPlayer`) |
...
| ID | 태스크 | 하는 일 | 관련 계약 | 선행 | 크기 |
|----|--------|--------|----------|------|------|
| U1 | 온보딩 화면 | 첫 실행 안내, 볼륨 카드(슬라이더), 방해금지 안내 | §3.10 | F2, F3, F5 | ✅ 완료 (`OnboardingScreen`·`SystemSetupHelper`) |
...
> ⚠️ **짚기 자체는 무음입니다** (2026-07-21 결정, SPEC §4) — 점만 찍히고 소리는 자동 스트로크가 냅니다.
>
> 1. **한 손가락**으로 아무 칸이나 누르기 → 그 자리에 점이 찍힘 (소리는 안 남 — 자동 주법이 냄)
> 2. **세 손가락으로 C코드 짚기**(5번줄 3프렛·4번줄 2프렛·2번줄 1프렛) → 점 3개, 자동 주법이 **화음**으로 긁음
> 3. 손가락을 **누른 채 옆 칸으로 밀기** → 점이 따라옴 (다음 스트로크부터 새 코드)
> 4. 짚은 채로 **다른 손가락 추가/제거** → 나머지는 계속 짚힌 상태로 유지
> 5. 같은 줄에 손가락 2개 → **높은 프렛만** 반영 (실제 기타와 동일)
> 6. 위쪽 **컨트롤 바를 눌러도** 지판 밖 터치는 무시됨
>
> ⚠️ 렌더링·좌표는 시뮬레이터로 확인 완료. 남은 건 **터치 입력**뿐입니다.
| U3 | 스트럼 화면 + 줄 애니메이션 | HI-FI + `notePlayed` 구독 → **줄 떨림·세기별 진폭/글로우**. ⚠️**iPhone·iPad 레이아웃 2종** (Figma `iPad Pro 12.9" - 3/4/5` 참고, 비율이 달라 늘리면 안 됨) | §3.4, §3.7 | F2, F3, C3 | 🟡 입력만 완료 |

> **U3 진행 상황** — **멀티터치 입력은 끝났고**(여러 줄 동시 튕김·아르페지오), 남은 건 둘입니다:
> 1. **줄 애니메이션** — `notePlayed` 구독 → 줄 떨림·세기별 진폭/글로우 (H1 햅틱과 같은 이벤트라 짝으로)
> 2. **iPad 레이아웃** — 비율이 달라 iPhone 것을 늘리면 안 됨 (ARCHITECTURE §2.5 / SPEC §8 결정 대기)
...
| U5 | 진행 선택 화면 | 프리셋 진행 목록 → 선택·확정 | §3.6 | F2, F3, C5 | ✅ 완료 (`ProgressionSelectScreen`) |
| U6 | 진행 커스텀 화면 | 카탈로그에서 코드 배치 + **미리듣기** + 결정 | §3.6 | F2, F3, C5 (미리듣기 완성엔 L4) | ✅ 완료 (`ProgressionCustomScreen` · L4 미리듣기 실동작) |
...
| H2 | Core Haptics 감쇠 | `UIImpactFeedbackGenerator` → `CHHapticEngine` 승격. 연속 이벤트 + 강도 감쇠 곡선으로 "줄이 잦아드는" 진동 | §3.11 | H1 | ✅ 구현 완료 (`DecayHapticPlayer`) · 감쇠 체감은 실기기 |
| H3 | 판정 햅틱 | 목표 코드 vs 현재 운지 비교 → 틀리면 강한 진동. 적용 범위는 SPEC §8 결정 대기 | §3.11, §3.3 | C3, H1 | ✅ 메커니즘 완료 (`ChordJudge`·`ChordJudgmentController`) · 켤 모드는 §8 대기 |
...
| I3 | 모드 C 조립 | iPhone 짚기 × iPad 긁기 → 두 기기 검증 | N1, U7, I1, I2 | 🟢 화면 연결까지 완료 (단일 세션·무음+진동·iPad 스트로크전용·연결배너·재연결) · **2기기 실기기 검증** 남음 |
