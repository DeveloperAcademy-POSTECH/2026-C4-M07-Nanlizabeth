...
flowchart LR
    ...
    P1 --> P2N["통신 레인 (N)"]
    P1 --> P2V["음성 레인 (V)"]
    P2V --> P3["Phase 3<br/>모드 조립·통합<br/>(I 태스크)"]
    P2A --> P3
    P2L --> P3
    P2C --> P3
    P2U --> P3
...
### Phase 1 — 계약 + Mock (C) ※산출물 = 껍데기 + Mock 1개 + 사용 예시 주석
...
| C8 | 음성 명령 계약 | `VoiceCommandService` 껍데기 + Mock(디버그 명령 버튼 패널) | §3.12 | — | S |
...
| A4 | 오디오 세션 안정성 | 전화·이어폰·Siri·블루투스 등 중단/경로변경 시 엔진 복구(`AVAudioSession` 알림). **안 하면 실사용 중 소리가 죽음** | §3.1 | — | M |
...
| ID | 태스크 | 하는 일 | 관련 계약 | 선행 | 크기 |
|----|--------|--------|----------|------|------|
| CT1 | 주법 프리셋 입력 | 초보자용 스트로크 리듬 리서치(칼립소·고고 등) → `StrumStep` 표로 입력 + 프리셋마다 **음성 별칭**(부르는 이름) 정하기 | §3.5, §3.12 | C4 | M |
| CT2 | 진행 프리셋 입력 | 머니코드(C→G→Am→F)·캐논 등 리서치 → 데이터 입력 + **음성 별칭** 정하기 | §3.6, §3.12 | C5, CT3 | M |
...
| U2 | 기타넥 화면 | HI-FI + **멀티터치 → `FretPress` 방출** + 짚힌 자리 표시 | §3.4 | F2, F3, C3 | L |
| U3 | 스트럼 화면 + 줄 애니메이션 | HI-FI + `notePlayed` 구독 → **줄 떨림·세기별 진폭/글로우** | §3.4, §3.7 | F2, F3, C3 | L |
| U4 | 스트로크 선택 화면 | 프리셋/커스텀 목록 → 선택·확정 | §3.5 | F2, F3, C4 | M |
| U5 | 진행 선택 화면 | 프리셋 진행 목록 → 선택·확정 | §3.6 | F2, F3, C5 | M |
| U6 | 진행 커스텀 화면 | 카탈로그에서 코드 배치 + **미리듣기** + 결정 | §3.6 | F2, F3, C5 (미리듣기 완성엔 L4) | L |
| U7 | 연결 플로우 화면 | 가이드(스킵 가능) + 근처 기기 찾기 + 권한 팝업 예고 | §3.8 | F2, F3, C7 | M |
| H1 | 손맛(햅틱) 연결 | `notePlayed`/`strumPerformed` → 세기별 햅틱. `HapticsManager` **이미 있음, 연결만.** U3와 같은 이벤트라 짝으로 | §3.11 | C3, C4 | S |
...
**🎤 음성 레인 (V)** — 핵심 기능. **V1(리스크 검증)은 계약 없이도 오늘부터 가능**

| ID | 태스크 | 하는 일 | 관련 계약 | 선행 | 크기 |
|----|--------|--------|----------|------|------|
| V1 | 인식 스파이크 ⚠️최우선 | 실기기에서 **스피커로 기타 소리를 내면서** 마이크 음성 인식이 되는지 검증 (에코 제거 vs 기타 음질 영향 포함). 실패 시 폴백(듣기 토글 등) 결정 | SPEC §5.1 | — | M |
| V2 | 음성 인식 구현 | 좁은 명령어 사전 + 프리셋 별칭 인식 → `VoiceCommand` 방송. 세션·화면에 연결 | §3.12 | C8, V1 | M |
| V3 | 권한·설정 연결 | Info.plist 키 2개(마이크·음성인식) + 권한 요청 UX(거부해도 앱 정상) + 듣기 토글 UI | §3.12 | V2 | S |
...
| I4 | 통합 QA | 전 화면 디자인 점검 + 실기기 발열/성능 + **연주 중 음성 전환·햅틱 실사용 검증** + 엣지 케이스(SPEC §8 결정 반영) | I1~I3, V2 | M |
...
gantt
    ...
    section 음성 레인
    V1 인식 스파이크 (리스크 먼저) :crit, v1, 2026-07-20, 2d
    V2·V3 음성 구현·권한          :v2, after v1, 3d
...
**글자로만 보일 때 읽는 법:** 기반(F) 며칠 → 계약(C) 이틀 안에 몰아서 → 그 순간부터 **소리/로직/콘텐츠/화면/통신/음성 6개 레인이 동시에** → 끝에서 모드 A→B→C 순서로 조립 → 맨 마지막에 엔진 결정.
...
| **음성 명령** | 연주 중 두 손이 바쁠 때, **말로** 프리셋을 바꾸는 핵심 기능 ("머니코드로 바꿔") |
| **스파이크** | 본격 개발 전에 "이게 기술적으로 되나?"만 빨리 실험하는 짧은 검증 작업 |
| **에코** | 스피커에서 나간 소리가 마이크로 되들어오는 것 — 음성 인식의 최대 적 |
