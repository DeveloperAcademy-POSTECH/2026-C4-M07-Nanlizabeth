...
## 12. 구현 현황 (최종 — 이 브랜치에서 실제로 만든 것)

MVP(§1~§4)를 넘어, 반복 피드백을 받으며 아래까지 완성했다. **원래 §11에서 "미구현"으로 뒀던
손가락 번호는 실제로 넣었다.**

- **노래 선택** — 드릴 버튼(넥 상단 컨트롤바, 연결 버튼 왼쪽 🎯)을 누르면 바로 시작하지 않고
  `ChordDrillSongSelectScreen`에서 곡을 고른다. 곡 데이터는 저작권 없는 동요·전통곡
  (`Content/PracticeSongData.swift`: 떴다떴다 비행기·반짝반짝 작은 별·학교종·산토끼·Amazing
  Grace·House of the Rising Sun). 카탈로그 8코드로만 구성. → `Domain/Session/PracticeSong.swift`
- **노래마다 피킹 패턴** — 코드를 통째로 긁지 않고 노래별 순서로 줄을 하나씩 튕긴다
  (아르페지오·베이스스트럼·왈츠). `Domain/Strum/PickPattern.swift` · `PickPatternPlayer.swift`.
  베이스(`.bass`)는 코드마다 **가장 낮은 울리는 줄**로 해석(`GuitarFingering.lowestAudibleString`).
- **목표 코드 넥 오버레이 + 손가락 번호** — 짚을 자리를 라임 고스트로 표시하고 손가락 번호(1~4)를
  칩 위에 안내. `ChordCatalog`에 `fingers` 데이터 추가. → `NeckGeometry.chordDiagramMarkers`
- **바레/개별 표시** — 같은 손가락이 3줄 이상 이으면 타원(바레), 아니면 원으로 나눠 표시
  (F의 바깥 두 줄은 원). 짚기 피드백은 줄마다 사운드홀 쪽(최고 프렛) 하나만.
- **바레 입력** — `UITouch.majorRadius`(접촉 면적)로 넓게 누르면 세로 인접 여러 줄로 확장
  (`MultiTouchLayer` → `TouchSample`, `NeckGeometry.presses(at:majorRadius:)`). 감도 상수
  `NeckGeometry.barreRadiusScale`.
- **안 치는 줄은 안 울림** — 목표 코드 보이싱으로 소리·줄 떨림을 내, 뮤트(X) 줄은 조용
  (`PlaySession.voicingOverride`). 판정은 짚은 운지로, 울림은 보이싱으로 분리.

핵심 계약 변경: `PlaySession`에 `soundGate` · `voicingOverride` 훅 추가(둘 다 기본 `nil` →
기존 모드 A·B·C 무변화). → ARCHITECTURE §3.7 갱신 대상.

---

## 13. 향후 아이디어 — 롱탭 A/B 테스트 (미구현, 계획만)

> 이번 브랜치엔 **넣지 않는다.** 다음에 실험할 때를 위한 기록.

### 왜 나온 아이디어
그냥 툭 터치하면 손을 약하게 짚고 넘어가는 경우가 있어, **오래 쥐게 해서 더 의도적으로 짚게**
만들자는 의견. 실제 코드 운지처럼 "잡고 버티는" 감각을 유도하려는 것.

### ⚠️ 먼저 알아둘 사실 — 아이폰은 "세기"를 못 잰다
- 3D Touch(압력 감지)는 iPhone XS 이후 사라졌고, 지금 **햅틱 터치 = "얼마나 오래 눌렀나(시간)"**
  만 잰다. 아이패드는 압력 센서가 애초에 없다. 정전식 터치는 **닿음 여부**만 안다.
- 그래서 **롱탭은 "힘"이 아니라 "시간·안정성"을 재는 프록시**다. "오래 쥐면 힘이 더 들어간다"는
  물리적으로 강제되지 않는다(살짝 댄 채 가만히 있어도 롱탭 성립). 이 전제는 그대로 쓰면 안 된다.

### 그래도 얻는 것 (프록시로서)
약한/실수 터치를 거르고, 손 모양을 **의도적으로 잡고 버티게** 유도. 지금 드릴은 이미 0.3초
`settle` 후 판정하므로, 롱탭 버전은 이를 더 길게(예: 0.7~1초) + **차오르는 링**으로 시각화하는 것.

### A/B 설계
- **A안 (롱탭 없음):** 올바른 모양 되는 즉시 통과·다음. 빠르고 반복 많음 → **전환 속도 훈련**·재미에 유리. 대충 넘기며 치팅 가능.
- **B안 (롱탭):** 링이 찰 때까지 유지해야 통과. 의도적·안정적 운지 유도·약한 터치 거름. 대신 느리고 흐름이 끊길 수 있음.
- **핵심 긴장:** 이 기능의 원래 목표는 **"빠른 왼손 코드 전환"** — 롱탭은 매 코드를 붙잡게 해
  속도 훈련과 상충한다. "정확·안정 vs 속도·재미" 중 무엇을 택할지가 실험의 쟁점.

### 측정 지표
완주율/이탈률 · 곡당 소요시간 vs 정확도 · 주관 만족("실제 연습 같았다"/"재밌었다") · 약한·실수 터치 비율.

### 대안 (더 목적에 맞을 수 있음)
"세기"를 흉내 내려면 **시간(롱탭)보다 접촉 면적(`majorRadius`, 이미 바레용으로 읽는 중)**이
낫다 — 꾹 눌러 손가락이 납작해질수록 접촉 면적이 커지므로 "제대로 짚었는지"의 거친 프록시가 된다.

### 구현 방향 (실험 시)
같은 드릴 화면에 **플래그 하나(롱탭 on/off + 차오르는 링)**로 A/B를 토글. 판정은
`ChordDrillController`의 `settleDelay`를 늘리고 진행 링을 붙이는 정도로 시작 가능.
