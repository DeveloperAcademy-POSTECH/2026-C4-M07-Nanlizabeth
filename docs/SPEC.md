...
| 음성 명령용 마이크·음성인식 권한 | ✅ 가능 — 표준 권한 팝업 2개 (Info.plist 문구 필요 → ROADMAP V3) | 온보딩에서 예고, 첫 음성 사용 시 요청. **거부해도 앱은 정상 동작**(음성만 꺼짐) |

> 💡 결론: 온보딩에서 사용자에게 부탁할 것은 **볼륨 · 방해금지 · (음성 쓸 경우) 권한 허용** 셋이고, 전부 **안내 + 표준 팝업** 방식입니다. 나머지는 앱이 알아서 처리합니다.
...
## 5. 음성 명령과 햅틱 ★핵심 기능 (연주를 멈추지 않기 위한 두 가지)

**왜 핵심인가:** 연주 중엔 **두 손이 다 악기에 붙어 있다** (왼손=넥, 오른손=스트로크). 화면 버튼을 누르려면 연주를 멈춰야 한다. 그래서 —

- 🎤 **입(음성)이 세 번째 손**: 연주를 멈추지 않고 말로 프리셋을 바꾼다.
- 🫨 **손끝(햅틱)이 두 번째 귀**: 진짜 기타처럼 튕김이 손에 울린다.

### 5.1 음성 명령 (Voice Control)

| 상황 | 말하는 예 | 일어나는 일 |
|------|----------|------------|
| 스트로크 연습 중 (모드 B) | "머니코드로 바꿔" / "캐논" | 코드진행이 그 프리셋으로 교체 — **연주는 계속** |
| 코드 연습 중 (모드 A) | "칼립소로 바꿔" / "고고" | 자동 스트로크 주법이 교체 — **연주는 계속** |

- **명령어 사전은 일부러 작게**: 프리셋 이름(+음성 별칭) 위주의 좁은 사전이라야 인식률이 높다. → 콘텐츠 태스크(CT1·CT2)에서 프리셋마다 **"부르는 이름(별칭)"**을 함께 정한다.
- **권한**: 마이크 + 음성인식, 표준 시스템 팝업 2개 (플로우 4 표). **거부해도 앱은 정상 동작** — 음성 기능만 꺼진다.
- ⚠️ **최대 기술 리스크 (스파이크 먼저!)**: 스피커로 기타 소리를 내는 **동시에** 마이크로 들어야 한다(에코 문제). iOS 에코 제거(voice processing)를 켜면 기타 소리 품질에 영향을 줄 수 있다 → **실기기 검증이 모든 음성 작업보다 먼저** (ROADMAP V1). 실패 시 폴백(예: 듣기 토글) 결정.
- 계약 명세: [ARCHITECTURE §3.12](ARCHITECTURE.md).
...
Figma **HI-FI** 페이지가 정답지입니다. 의도가 안 잡히면 **LO-FI**를 보조로 보세요.
(⚠️ 아래 이름은 코드 기준 임시 명칭 — Figma 접근되면 HI-FI 페이지명과 1:1로 맞출 것 → ROADMAP 태스크 F2)

| # | 화면 | 하는 일 | 관련 플로우 | 현재 코드 |
|---|------|--------|-----------|----------|
| 1 | 온보딩 | 첫 실행 안내 + 볼륨/방해금지 세팅 유도 | 4 | ⛔ 없음 |
| 2 | 기타넥 (iPhone 메인) | 코드 짚기, 핑거보드 개별 발음 | 1, 3 | 🟡 `ScreenshotFretboardView` (탭만) |
| 3 | 스트럼 (iPad 메인 / iPhone 모드B) | 줄 긁기 + 줄 애니메이션 | 2, 3 | 🟡 `GuitarStrumView` (소리는 됨, 줄은 정지 그림) |
| 4 | 스트로크 선택 | 프리셋 주법 목록 → 선택·확정 | 1 | 🟡 더미 목록 |
| 5 | 스트로크 생성 | 커스텀 주법 만들기 (범위 미정 → §8) | 1 | 🟡 타임라인만 |
| 6 | 코드진행 선택 | 프리셋 진행 목록 | 2 | 🟡 하드코딩 |
| 7 | 코드진행 커스텀 | 카탈로그에서 코드 골라 배치 + 미리듣기 + 결정 | 2 | ⛔ 없음 |
| 8 | 연결 가이드 | 첫 연결 안내(스킵 가능) | 3 | ⛔ 없음 |
| 9 | 근처 기기 찾기 | 탐색·초대·연결 상태 | 3 | 🟡 버튼만 (`PeerLiquidGlassButton`) |
...
> ✅ **이미 결정된 것 (기록):** 초보자 튜토리얼(연주법 가르치기)은 **만들지 않는다** · 왼손잡이 반전 옵션은 **뺀다** · **음성 명령과 햅틱은 핵심 기능이다**(§5).

- [ ] **모드 A에서 아무 코드도 안 짚었을 때**: 개방현으로 긁나, 무음인가?
- [ ] **연결이 끊겼을 때**: 연주 일시정지? 단독 모드로 자동 전환? 알림만?
- [ ] **카운트인**: 스트로크 연습 시작 전 "하나, 둘, 셋, 넷" 박자 예고를 넣을지
- [ ] **스트로크 생성(커스텀 주법) 화면의 범위**: LO-FI에 있는데 HI-FI 확정 필요. 1차 출시에 포함?
...
- [ ] 🎤 **음성 명령어 최종 목록**: 프리셋 전환 외에 재생/정지·BPM 변경도 말로 할지
- [ ] 🎤 **오인식 처리**: 못 알아들었거나 잘못 알아들었을 때 화면 피드백을 어떻게 줄지
- [ ] 🎤 **듣기 방식**: 항상 듣기 vs 토글로 켜고 끄기 (배터리·프라이버시)
