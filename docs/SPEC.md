...
## 4. 짚기는 무음, 소리는 자동 스트로크가 낸다 (모드 A) ★기술 핵심

> 🔁 **결정 변경 (2026-07-21):** 원래는 "핑거보드 칸이 눌리면 그 즉시 개별 소리가 난다"(개별 발음)였으나, **실사용에서 "짚을 때마다 소리가 나는 게 연습에 방해된다"**는 판단으로 뒤집었다. 아래가 현재 동작이다. (아래 ~~취소선~~이 옛 방식.)

**모드 A의 넥에서 코드를 짚어도 소리는 나지 않는다.** 사용자는 **그저 코드를 잡을 뿐**이고, 소리는 **선택한 주법(스트로크)이 BPM에 맞춰 시간이 흐름에 따라 자동으로 긁으며** 그 순간 짚은 코드로 낸다. (스트로크 모드에서 코드진행이 시간에 따라 자동으로 도는 것과 **대칭**이다.)

- **짚기** = 무음의 코드 홀드. 손가락 위치(점)만 표시된다.
- **소리** = 자동 주법이 매 박 긁을 때. 코드를 바꿔 짚으면 **다음 스트로크부터** 바뀐 코드로 난다.
- **멀티터치는 여전히 필요하다** — 화음은 여러 칸을 동시에 짚어야 하기 때문. 다만 짚는 순간 소리가 안 날 뿐 ([ADR 0006](adr/0006-uikit-multitouch-input.md) 유효).
- 연결(모드 C)이면 짚은 운지를 iPad로 보내고, 소리는 iPad의 스트로크가 낸다 (§2 모드 C).

> ~~스트로크가 자동이거나(모드 A) 아이패드가 맡은(모드 C) 상황에서, 아이폰 기타넥 화면의 핑거보드 칸 하나하나가 눌리면 그 즉시 소리가 나야 한다.~~ ← **옛 개별 발음 방식. 위 결정으로 대체됨.**
...
|---|------|--------|-------|------------------|----------|
| 1 | 온보딩 | 첫 실행 안내 + 볼륨/방해금지 세팅 유도 | 4 | `iPhone 17 - 9 / 11 / 23` (3장, 이름 미지정) | ✅ `OnboardingScreen` (U1) |
| 2 | 기타넥 (iPhone 메인) | 코드 짚기(무음) + 자동 스트로크가 그 코드로 연주 (§4) | 1, 3 | `코드 모드 - 기본` (+상태 6종) | ✅ `NeckScreen` (U2 — 멀티터치 실동작) |
...
- [ ] 🕐 **진행의 "지금 어디쯤"을 어떻게 보여줄지** (2026-07-21 사용 중 발견): 모드 B에서 코드진행이 BPM대로 자동으로 바뀌는데, **지금 무슨 코드인지·다음은 뭔지·마디 안 어디쯤인지**를 화면에서 알 방법이 없다. 시간이 흐르는 감각이 없어 사용자가 진행을 따라가기 어렵다. 필요한 피드백을 정하자 — 예: 현재 코드 라벨 / 다음 코드 미리보기 / 마디·박 인디케이터 / 카운트인(위 항목과 연계). **디자인 결정 사항.**
