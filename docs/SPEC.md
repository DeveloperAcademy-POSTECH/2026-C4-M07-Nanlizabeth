...
| 2 | 기타넥 (iPhone 메인) | 코드 짚기, 핑거보드 개별 발음 | 1, 3 | `코드 모드 - 기본` (+상태 6종) | 🟡 `ScreenshotFretboardView` (탭만) |
| 3 | 스트럼 (iPad 메인 / iPhone 모드B) | 줄 긁기 + 줄 애니메이션 | 2, 3 | `스트로크 모드` / `스트로크 모드 - 기본` | 🟡 `GuitarStrumView` (소리는 됨, 줄은 정지 그림) |
| 4 | 스트로크 선택 | 프리셋 주법 목록 → 선택·확정 | 1 | `스트로크 선택 페이지 - 선택안됨 / 선택됨` | 🟡 더미 목록 |
| 5 | 스트로크 생성 | 커스텀 주법 만들기 | 1 | ⛔ **HI-FI에 없음** → §8 결정 | 🟡 타임라인만 |
| 6 | 코드진행 선택 | 프리셋 진행 목록 | 2 | `코드 선택 페이지 - 선택됨` | 🟡 하드코딩 |
| 7 | 코드진행 커스텀 | 카탈로그에서 코드 골라 배치 + 미리듣기 + 결정 | 2 | `코드 커스텀 페이지` | ⛔ 껍데기만 |
| 8 | 연결 가이드 | 첫 연결 안내(스킵 가능) | 3 | ⛔ **HI-FI에 없음** → §8 결정 | ⛔ 껍데기만 |
| 9 | 근처 기기 찾기 | 탐색·초대·연결 상태 | 3 | `코드 모드 - 기기 연결 창 / 기기 연결됨` ⚠️**독립 화면이 아니라 오버레이** | ⛔ 껍데기만 |
...
- [ ] **스트로크 생성(커스텀 주법) 화면**: HI-FI에 **없는 것을 확인**(2026-07-20). 1차 출시에서 빼는 게 맞나? 뺀다면 스트로크 선택 화면의 "커스텀 +" 버튼도 함께 정리해야 한다
- [ ] **연결 가이드(화면 8)**: HI-FI에 **독립 화면이 없다.** Figma에는 연결이 `코드 모드` 위 오버레이(`기기 연결 창`)로만 있다. 가이드를 없애고 오버레이만 갈지, 첫 회 가이드를 별도로 만들지 결정 필요 (SPEC 플로우 3은 가이드 단계를 전제하고 있음)
- [ ] **iPad 전용 레이아웃**: HI-FI에 iPad Pro 12.9"(1366×1024) 프레임이 따로 있다. iPhone(874×402)과 가로세로비가 2.17 : 1.33으로 크게 달라 **"기준 해상도 하나 + 비율 스케일"(F1 결정)로는 커버가 안 된다.** iPad를 1차 범위에 넣을지, 넣는다면 레이아웃 분기를 허용할지 결정 필요
